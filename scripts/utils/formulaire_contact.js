...
document.getElementById("ouvrirModal").addEventListener("click", (event) => {
    displayModal(event)
})
...
function displayModal() {
    ...
    const elementFocus = document.querySelectorAll("#contact_modal input, #contact_modal textarea, #contact_modal img, #contact_modal button")
    ...
    modal.style.display="block"
    document.querySelector("#contact_modal").addEventListener("keydown", (event) => {
        const actuel = event.target
        if (event.key === "Escape" || (event.key === "Enter" && actuel == premierElement)) {
            event.preventDefault()
            closeModal()
